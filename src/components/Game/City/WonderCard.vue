<template>
  <Cover :size="size" :group="group">
    <q-tooltip anchor="top middle" self="bottom middle" class="transparent">
      <Card :id="id" />
    </q-tooltip>
  </Cover>
</template>
...
const group = (function getGroup(cid: CardId) {
  const char = cid.toString()[0];

  return +char < 4 ? `age_${char}` : 'guild';
})(id);
</script>
